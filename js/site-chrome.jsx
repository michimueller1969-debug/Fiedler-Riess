...
(function () {
  ...
  function navItems(current) {
    ...
    return [
      ...
      { label: 'Rechtsgebiete', href: 'index.html#practice', active: rechtsgebieteActive, children: [
        { label: 'Arbeitsrecht', href: 'index.html#practice' },
        { label: 'Verkehrsrecht', href: 'index.html#verkehrsrecht', children: [
          { label: 'Fragebogen', href: 'index.html#fragebogen' },
        ]},
        { label: 'Jagdrecht', href: 'index.html#jagdrecht' },
      ]},
      { label: 'Karriere', href: 'index.html#karriere' },
      { label: 'Anfahrt', href: 'index.html#anfahrt' },
    ];
  }

  /* ---- Desktop-Navigationsleiste (links, über dem Hero schwebend) ---- */
  function NavRail({ items, variant }) {
    const transparent = variant === 'transparent';
    return (
      <nav style={{ background: transparent ? 'transparent' : 'var(--charcoal)', width: transparent ? 'auto' : 'var(--nav-w)', display: transparent ? 'inline-block' : 'block' }}>
        <ul style={{ listStyle: 'none', margin: 0, padding: 0, paddingTop: 6, display: 'flex', flexDirection: 'column' }}>
          {items.map((item, i) => <NavItem key={i} item={item} transparent={transparent} />)}
        </ul>
      </nav>
    );
  }
  function NavItem({ item, transparent }) {
    ...
    return (
      <li style={{ position: 'relative' }} onMouseEnter={enter} onMouseLeave={leave}>
        <a href={item.href || '#'} style={{ display: 'block', padding: transparent ? '14px 22px' : '11px 16px', color: itemColor, fontFamily: 'var(--font-sans)', fontSize: transparent ? 17 : 12.5, fontWeight: 'var(--fw-semibold)', letterSpacing: 'var(--ls-nav)', textTransform: 'uppercase', whiteSpace: 'nowrap', textDecoration: 'none', borderRight: item.active ? '3px solid var(--green)' : '3px solid transparent', transition: 'background var(--ease)' }}>
          {item.label}{hasKids ? ' ›' : ''}
        </a>
        {hasKids && (
          <ul className="fr-dropdown" style={{ display: 'none', position: 'absolute', top: 0, left: '100%', minWidth: 200, background: ddBg, borderLeft: transparent ? 'none' : '2px solid var(--green)', zIndex: 300, listStyle: 'none', margin: 0, padding: 0 }}>
            {item.children.map((c, j) => <SubItem key={j} item={c} transparent={transparent} ddBg={ddBg} subColor={subColor} />)}
          </ul>
        )}
      </li>
    );
  }
  function SubItem({ item, transparent, ddBg, subColor }) {
    const hasKids = item.children && item.children.length > 0;
    const enter = (e) => { const dd = e.currentTarget.querySelector(':scope > .fr-dropdown'); if (dd) dd.style.display = 'block'; };
    const leave = (e) => { const dd = e.currentTarget.querySelector(':scope > .fr-dropdown'); if (dd) dd.style.display = 'none'; };
    return (
      <li style={{ position: 'relative' }} onMouseEnter={enter} onMouseLeave={leave}>
        <a href={item.href || '#'} style={{ display: 'block', padding: transparent ? '12px 22px' : '9px 16px', color: subColor, fontFamily: 'var(--font-sans)', fontSize: transparent ? 16 : 12.5, textDecoration: 'none', whiteSpace: 'nowrap', transition: 'background var(--ease)' }}>
          {item.label}{hasKids ? '  ›' : ''}
        </a>
        {hasKids && (
          <ul className="fr-dropdown" style={{ display: 'none', position: 'absolute', top: 0, left: '100%', minWidth: 200, background: ddBg, borderLeft: transparent ? 'none' : '2px solid var(--green)', zIndex: 300, listStyle: 'none', margin: 0, padding: 0 }}>
            {item.children.map((c, k) => <SubItem key={k} item={c} transparent={transparent} ddBg={ddBg} subColor={subColor} />)}
          </ul>
        )}
      </li>
    );
  ...
  function MobileNavItem({ item, depth }) {
    ...
    const labelClick = (e) => { if (hasKids) { e.preventDefault(); setOpen((o) => !o); } };
    return (
      <li style={{ listStyle: 'none' }}>
        <div style={{ display: 'flex', alignItems: 'stretch', borderBottom: '1px solid rgba(255,255,255,.08)' }}>
          <a href={item.href || '#'} onClick={labelClick}
            style={{ flex: 1, padding: `15px ${pad}px`, color: item.active ? 'var(--green)' : '#fff', fontFamily: 'var(--font-sans)', fontSize: depth === 0 ? 16 : 15, fontWeight: 'var(--fw-semibold)', letterSpacing: '.05em', textTransform: 'uppercase', textDecoration: 'none' }}>
            {item.label}
          </a>
          {hasKids && (
        ...
        {hasKids && open && (
          <ul style={{ margin: 0, padding: 0, background: 'rgba(0,0,0,.18)' }}>
            {item.children.map((c, i) => <MobileNavItem key={i} item={c} depth={depth + 1} />)}
          </ul>
        )}
      </li>
    );
  }
  function MobileNav({ items, open }) {
    return (
      <div style={{ overflow: 'hidden', maxHeight: open ? '80vh' : 0, transition: 'max-height .3s ease', background: 'var(--charcoal)', borderTop: open ? '3px solid var(--green)' : 'none' }}>
        <ul style={{ margin: 0, padding: 0, listStyle: 'none', overflowY: 'auto', maxHeight: '80vh' }}>
          {items.map((it, i) => <MobileNavItem key={i} item={it} depth={0} />)}
        </ul>
      </div>
    );
  }

  function HeaderBrand({ isMobile, menuOpen, onToggle }) {
    return (
      <header style={{ background: 'var(--white)' }}>
        <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: isMobile ? '20px 16px 18px' : '44px 20px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <a href="index.html" style={{ textDecoration: 'none', textAlign: 'left', borderLeft: '1px solid var(--border)', paddingLeft: isMobile ? 14 : 22 }}>
  ...
  function Hero() {
    ...
    return (
      <div style={{ height: isMobile ? 180 : 310, position: 'relative', overflow: 'hidden', background: '#4a4a4a' }}>
        {slides.map((s, i) => (
          <div key={i} style={{ position: 'absolute', inset: 0, backgroundImage: `url('${s}')`, backgroundSize: 'cover', backgroundPosition: 'center 30%', backgroundRepeat: 'no-repeat', transform: i === cur ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 1.1s ease', opacity: i === cur ? 1 : 0 }} />
        ))}
      </div>
    );
  }
      ...
      <div style={{ background: 'var(--white)' }}>
        <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: isMobile ? '26px 16px 4px' : '34px 20px 8px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'flex-start', gap: isMobile ? 18 : 50 }}>
          <div style={{ maxWidth: 760, flex: 1 }}>
            <h1 style={{ font: `300 ${isMobile ? '26px' : '30px'} var(--font-sans)`, color: 'var(--text)', display: 'inline-block', paddingBottom: 8, borderBottom: '2px solid var(--green)', margin: 0 }}>{title}</h1>
  ...
  function Footer() {
    return (
      <footer style={{ background: 'var(--white)', borderTop: '1px solid var(--border)', padding: '15px 0', marginTop: 50 }}>
        <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
          ...
          <nav style={{ display: 'flex', gap: 20 }}>
            <a href="index.html#impressum" style={{ color: 'var(--green)', font: '400 13px var(--font-sans)' }}>Impressum</a>
            <a href="index.html#datenschutz" style={{ color: 'var(--green)', font: '400 13px var(--font-sans)' }}>Datenschutz</a>
          </nav>
        </div>
      </footer>
    );
  ...
  /* ---- Gesamte Chrome ---- */
  function SiteChrome({ current, title, tagline, children }) {
    const isMobile = useIsMobile();
    ...
    return (
      <div style={{ position: 'relative', minHeight: '100%', background: 'var(--white)', display: 'flex', flexDirection: 'column' }}>
        <HeaderBrand isMobile={isMobile} menuOpen={menuOpen} onToggle={() => setMenuOpen((o) => !o)} />
        {isMobile && <MobileNav items={items} open={menuOpen} />}
            ...
            <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, zIndex: 200, pointerEvents: 'none' }}>
              <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '0 20px', height: '100%' }}>
                <div style={{ display: 'inline-block', height: '100%', background: 'rgba(245,245,243,0.62)', pointerEvents: 'auto' }}>
                  <NavRail items={items} variant="transparent" />
                </div>
              </div>
            </div>
          )}
        ...
        {title && <PageHeader title={title} tagline={tagline} />}
